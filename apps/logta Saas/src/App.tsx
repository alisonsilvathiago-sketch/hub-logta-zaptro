...
const Sidebar = () => {
  ...
  return (
    <aside 
      className="h-[calc(100vh-32px)] m-4 bg-white border border-gray-100 flex flex-col sticky top-4 transition-all duration-300 ease-in-out z-20 rounded-[24px] shadow-xl shadow-gray-200/50 w-24"
    >
      <div className="mt-6 mb-6 flex flex-col items-center transition-all px-4">
        <div className="relative transition-all duration-300 w-10 h-10">
          <img 
            ...
            alt="User" 
            className="rounded-full border-2 border-gray-50 shadow-sm object-cover w-full h-full"
          />
      ...
      <nav className="flex-1 space-y-2 px-4">
        {menuItems.map((item) => {
          const isActive = location.pathname.includes(item.path) && (item.path !== '/' || location.pathname === '/');
          return (
            <Link 
              key={item.label}
              to={item.path}
              className={`flex items-center justify-center p-3 rounded-2xl transition-all group relative
                ${item.label === 'Início'
            ...
            >
              <item.icon size={18} className={`flex-shrink-0 ${(isActive || item.label === 'Início') ? 'text-white' : 'group-hover:text-primary'} transition-colors`} />
              
              {/* Premium Floating Tooltip */}
              <div className="absolute left-full ml-4 px-3 py-1.5 bg-gray-950 text-white text-[10px] font-black uppercase tracking-normal rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:translate-x-1 translate-x-0 transition-all duration-200 whitespace-nowrap z-50 shadow-xl shadow-gray-950/20">
                {item.label}
      ...
      <div className="p-4 border-t border-gray-50 px-4">
        <button className="flex items-center justify-center w-full p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all group relative cursor-pointer">
          <LogOut size={22} className="flex-shrink-0" />
          
          {/* Premium Logout Tooltip */}
          <div className="absolute left-full ml-4 px-3 py-1.5 bg-red-600 text-white text-[10px] font-black uppercase tracking-normal rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:translate-x-1 translate-x-0 transition-all duration-200 whitespace-nowrap z-50 shadow-xl shadow-red-600/10">
            Sair do Sistema
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2 space-y-1">
                      <button 
                        onClick={() => {
                          setIsHelpOpen(false);
                          (window as any).showToast?.('info', 'IA Operacional ativada no canto inferior esquerdo!', 'Logta AI');
                        }}
        ...
        <div className="relative">
          ...
          >
            <div className="text-right">
              <p className="text-sm font-semibold text-gray-900">Alison Thiago</p>
              <p className="text-[10px] text-gray-500 font-bold uppercase">Administrador</p>
            </div>
            <div className="w-10 h-10 bg-gradient-to-tr from-primary to-blue-600 rounded-full border-2 border-white/50 shadow-sm" />
          </div>
    ...
    <div className="flex h-screen bg-[#F9FAFB] overflow-hidden text-gray-900 font-sans selection:bg-primary/30 selection:text-gray-900">
      ...
      <main className="flex-1 flex flex-col h-screen min-w-0">
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-8 pt-4 scrollbar-hide">
          <div className="bg-white min-h-full flex flex-col rounded-[40px] shadow-sm border border-gray-100 overflow-y-auto scrollbar-hide relative">
            <Routes>
