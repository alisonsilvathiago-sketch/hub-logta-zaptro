  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white border border-gray-100 p-6 rounded-[32px] shadow-sm relative overflow-hidden group hover:border-primary/20 transition-all text-left">
            <div className="relative z-10">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-normal mb-1">{stat.label}</p>
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-[40px] p-8 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <div className="text-left">
              <h3 className="text-xl font-bold text-gray-900 tracking-tight">Monitoramento de Risco em Tempo Real</h3>
          ...
          <div className="space-y-4">
            {activeAlerts.length === 0 ? (
              <div className="p-12 text-center bg-gray-50 rounded-[32px] border border-dashed border-gray-200">
                ...
                <p className="font-bold text-gray-500">Nenhum alerta crítico ativo no momento.</p>
              </div>
            ) : (
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white border border-gray-200 rounded-[40px] p-8 shadow-sm">
          <h3 className="text-xl font-bold text-gray-900 mb-8 tracking-tight text-left">Regras Gerais do PGR</h3>
          <div className="space-y-4">
            {[
        ...
        <div className="bg-gray-900 rounded-[40px] p-8 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl">
          <div className="relative z-10 text-left">
            <h3 className="text-xl font-bold mb-4">Configuração de Macros</h3>
...
const SegurosView = () => {
  ...
  return (
    <div className="space-y-8 text-left relative">
      <div className="bg-white border border-gray-200 rounded-[40px] p-8 shadow-sm">
        <div className="flex justify-between items-center mb-8">
          <h3 className="text-xl font-bold text-gray-900 tracking-tight">Controle de Apólices e Endossos</h3>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {policies.map((p, i) => (
                ...
                >
                  <td className="py-6">
                    <p className="text-xs font-black text-gray-900 group-hover:text-primary transition-colors">{p.id}</p>
                  ...
                  <td className="py-6">
                    <p className="text-xs font-black text-gray-900">{p.lmg}</p>
                  </td>
                  <td className="py-6">
...
const PGR = () => {
  ...
  const tabs = [
    { id: 'dashboard', label: 'Monitoramento', icon: Activity, path: '/pgr/dashboard' },
    { id: 'regras', label: 'Plano de Risco (PGR)', icon: Lock, path: '/pgr/regras' },
    { id: 'seguros', label: 'Apólices & Seguros', icon: ShieldCheck, path: '/pgr/seguros' },
    { id: 'sinistros', label: 'Sinistros', icon: LifeBuoy, path: '/pgr/sinistros' },
  ];

  return (
    <div className="px-16 py-10 w-full h-full space-y-8 animate-in fade-in duration-700">
      <div className="flex justify-between items-end mb-12 h-[87px]">
        <div className="text-left">
          <h1 className="text-4xl font-black text-gray-900 tracking-tight italic h-[46px] mt-[74px]">PGR & Seguros</h1>
        </div>
        <div className="flex gap-3">
          <button className="px-6 py-3.5 bg-white text-gray-900 rounded-xl font-bold text-sm border border-gray-200 hover:bg-gray-50 transition-all shadow-sm flex items-center gap-2 cursor-pointer">
            <FileText size={16} /> Relatório de Risco
          </button>
          <button 
            onClick={() => setIsMonitoring(!isMonitoring)}
            className={`px-6 py-3.5 rounded-xl font-bold text-sm transition-all shadow-xl flex items-center gap-2 cursor-pointer ${
              isMonitoring 
                ? 'bg-green-500 text-white shadow-green-500/20' 
                : 'bg-gray-900 text-white shadow-black/10 hover:bg-black'
            }`}
          >
            <ShieldCheck size={16} className={isMonitoring ? 'text-white' : 'text-primary'} />
            {isMonitoring ? 'Monitoramento Ativo' : 'Ativar Monitoramento'}
          </button>
        </div>
      </div>

      <div className="flex items-center gap-2 p-1.5 bg-gray-100 rounded-[28px] w-fit overflow-x-auto">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <Link
              key={tab.id}
              to={tab.path}
              className={`flex items-center gap-3 px-8 py-4 rounded-[22px] font-black text-xs transition-all whitespace-nowrap uppercase tracking-normal
                ${isActive 
                  ? 'bg-white text-gray-900 shadow-xl shadow-black/5 scale-[1.02]' 
