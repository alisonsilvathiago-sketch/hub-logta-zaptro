...
CREATE POLICY "Self insert" ON public.profiles FOR INSERT WITH CHECK (auth.uid() = id);`}
  ...
  if (!isMaster) {
    ...
    const now = new Date();
    const trialExpired = company?.trial_end_date ? new Date(company.trial_end_date) < now : false;
    ...
    if ((isSuspended || isBillingBlocked || (trialExpired && isTrialing) || company?.status === 'blocked') && !isOnPaymentPath) {
