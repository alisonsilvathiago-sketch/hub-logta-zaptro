...
export const getContext = () => {
  ...
  if (hostname === 'localhost' || hostname === '127.0.0.1') {
    if (path.startsWith('/play')) return 'CHECKOUT';
  ...
  if (hostname === 'app.zaptro.com.br') return 'WHATSAPP';
  if (hostname === 'zaptro.com.br' || hostname === 'www.zaptro.com.br') return 'MARKETING';
  ...
  // Legado / Fallback
  if (hostname.includes('zaptro')) return 'WHATSAPP';
  
  return 'MARKETING'; 
};

export const enforceDomain = (_context: string, _path: string) => {
  // Body disabled for debugging infinite redirect loop
  /*
  ...
  // 🛡️ SAFEGUARD: Se já estiver no ecossistema Zaptro (root, www ou app), não redireciona para evitar loops.
  if (currentHostname.includes('zaptro.com.br')) return;
