...
const ZaptroTeamContent: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'members' | 'ranking' | 'permissions'>(
    (tabFromUrl === 'ranking' || tabFromUrl === 'permissions') ? tabFromUrl : 'members'
  );

  useEffect(() => {
    if (tabFromUrl && (tabFromUrl === 'members' || tabFromUrl === 'ranking' || tabFromUrl === 'permissions')) {
      setActiveTab(tabFromUrl);
    }
  }, [tabFromUrl]);

  const handleTabChange = (t: 'members' | 'ranking' | 'permissions') => {
    setActiveTab(t);
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      notifyZaptro('error', 'Não foi possível guardar', formatZaptroDbErrorForToast(e, 'Tente novamente ou verifique as permissões no Supabase.', profile?.role), {
        toastId: 'zaptro-equipe-membro-erro',
      });
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      notifyZaptro('error', 'Não foi possível remover', formatZaptroDbErrorForToast(e, 'Verifique as permissões no Supabase.', profile?.role), {
  ...
  const submitAddMember = async () => {
    ...
    try {
      // Nota: Em um fluxo real de Auth, isso seria feito via invite. 
      // Aqui estamos criando o perfil diretamente para "ativar" a função na interface.
      ...
      const { error } = await supabaseZaptro
        ...
        .insert({
          ...
          role: addForm.role,
          permissions: sanitizeZaptroPagePermissions(addForm.pagePerms),
          metadata: { email: addForm.email.trim() }
        });

      if (error) throw error;

      notifyZaptro('success', 'Colaborador adicionado', `${addForm.full_name} foi registrado na equipe.`);
      setAddOpen(false);
      setAddForm({ full_name: '', email: '', password: '', role: 'agent', pagePerms: assignablePageIds(company) });
      await loadMembers();
    } catch (e: unknown) {
      notifyZaptro('error', 'Não foi possível adicionar', formatZaptroDbErrorForToast(e, 'Verifique as permissões ou se o e-mail já existe.', profile?.role));
    }
  };
      ...
      >
        {(
          [
            ...
            { id: 'permissions' as const, label: 'Acessos', icon: ShieldCheck },
          ] as const
          ...
          {loading ? (
            ...
            <ol style={styles.rankingList}>
              {rankedMembers.map((m, idx) => {
                ...
                const score = 100 - idx * 2; // Simulated engagement score
                return (
