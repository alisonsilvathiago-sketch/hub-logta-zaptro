  ...
  const gatewayAction = async (action: string, body: any = {}) => {
    try {
      ...
      const { data, error } = await supabaseZaptro.functions.invoke('evolution-gateway', {
        body: { action, instanceName, companyId: profile?.company_id, ...body }
      });
      if (error) {
  ...
  const startPolling = () => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      ...
      if (res.value) {
        ...
        // Popup de sucesso
        notifyZaptro('success', 'WhatsApp Conectado', 'A sua conta foi vinculada com sucesso!');

        // Redirecionar automaticamente para a caixa de entrada
        setTimeout(() => navigate('/whatsapp', { state: { justConnected: true } }), 1800);
