...
const ZaptroVehicleProfile: React.FC = () => {
  ...
  const [rodizioOption, setRodizioOption] = useState('');
  const [docsDate, setDocsDate] = useState('10/2024');

  useEffect(() => {
    if (vehicle && vehicle.plate) {
      const lastDigit = parseInt(vehicle.plate.replace(/\D/g, '').slice(-1));
      let dia = 'Sexta-feira (9 e 0)';
      if (lastDigit === 1 || lastDigit === 2) dia = 'Segunda-feira (1 e 2)';
      else if (lastDigit === 3 || lastDigit === 4) dia = 'Terça-feira (3 e 4)';
      else if (lastDigit === 5 || lastDigit === 6) dia = 'Quarta-feira (5 e 6)';
      else if (lastDigit === 7 || lastDigit === 8) dia = 'Quinta-feira (7 e 8)';
      setRodizioOption(dia);
    }
  }, [vehicle]);
      ...
      <ZaptroLayout>
        <div style={{ padding: 48, textAlign: 'center' }}>
          ...
          <button 
            ...
            style={{ marginTop: 24, padding: '12px 24px', backgroundColor: 'rgba(217, 255, 0, 0.14)', color: '#000', borderRadius: 14, border: '1px solid rgba(217, 255, 0, 1)', fontWeight: 700, cursor: 'pointer' }}
          >
            Voltar para Frota
          </button>
        </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
                    ...
                    <div>
                       ...
                       <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#10B981' }}>OK · Vencimento {docsDate}</p>
                       <div style={{ height: 1, backgroundColor: border, margin: '16px 0' }} />
                       <label style={{ fontSize: 11, fontWeight: 900, color: palette.textMuted, display: 'block', marginBottom: 8 }}>RODÍZIO (SÃO PAULO)</label>
                       <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: palette.text }}>{rodizioOption}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      {editModalOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
               <div>
                  <label style={{ fontSize: 11, fontWeight: 800, color: palette.textMuted, display: 'block', marginBottom: 6 }}>DATA DE LICENCIAMENTO</label>
                  <input 
                    type="text" 
                    value={docsDate} 
               ...
               <div>
                  <label style={{ fontSize: 11, fontWeight: 800, color: palette.textMuted, display: 'block', marginBottom: 6 }}>DIA DE RODÍZIO (SP)</label>
                  <input 
                    type="text" 
                    value={rodizioOption} 
                    onChange={(e) => setRodizioOption(e.target.value)} 
