...
const SECTOR_LABELS = {
  LOGISTICA: { label: 'LOGÍSTICA', color: LIME, icon: Truck },
  FINANCEIRO: { label: 'FINANCEIRO', color: '#3b82f6', icon: DollarSign },
  COMERCIAL: { label: 'COMERCIAL', color: '#a855f7', icon: Briefcase },
  SUPORTE: { label: 'SUPORTE', color: '#f97316', icon: Info },
};
...
const WhatsAppPremiumContent: React.FC = () => {
  ...
  const isBillingBlocked = useMemo(() => {
    ...
    if (company.status === 'blocked' || company.status === 'SUSPENSO' || company.status === 'BLOQUEADO') return true;
    if (company.billing_status === 'blocked' || company.billing_status === 'overdue') return true;
    if (company.status === 'trial' && company.trial_end_date) {
      if (new Date(company.trial_end_date) < new Date()) return true;
    }
    if ((company.wa_credits ?? 0) <= 0) return true;
    return false;
  }, [company]);

  const { palette } = useZaptroTheme();
  const location = useLocation();
  const [showConnectedModal, setShowConnectedModal] = useState(location.state?.justConnected === true);
  ...
  const [contactClassification, setContactClassification] = useState<'FROTA' | 'COLABORADOR' | 'CLIENTE' | 'LEAD' | 'NOVO' | null>(null);
  const [classificationLoading, setClassificationLoading] = useState(false);
  const [contactExtendedData, setContactExtendedData] = useState<any>(null);
  const [currentSectorLayout, setCurrentSectorLayout] = useState<'LOGISTICA' | 'FINANCEIRO' | 'COMERCIAL' | 'SUPORTE'>('LOGISTICA');
  const [quickCallModalOpen, setQuickCallModalOpen] = useState(false);
  const [quickCallNumber, setQuickCallNumber] = useState('');
  const [quickCallChecking, setQuickCallChecking] = useState(false);
  ...
  const mergedDemoConversations: Conversation[] = [];

  const displayConversations = useMemo(() => {
    let raw = [...conversations];

    // Se temos um chat selecionado que é virtual (não está no DB ainda), incluímos na lista para aparecer no sidebar
    if (selectedChat && selectedChat.id.startsWith('virtual-') && !raw.find(c => c.id === selectedChat.id)) {
      raw = [selectedChat, ...raw];
    }
    ...
    return [...filtered].sort((a, b) => {
      ...
      const aDate = new Date(a.last_customer_message_at || a.created_at || 0).getTime();
      const bDate = new Date(b.last_customer_message_at || b.created_at || 0).getTime();
      return bDate - aDate;
    });
  }, [conversations, pinnedChatIds, deletedChatIds, selectedChat]);
  /**
  ...
  const identifyContact = async (phone: string) => {
    if (!profile?.company_id) return null;
    setClassificationLoading(true);
    const digits = phone.replace(/\D/g, '');
    
    try {
      // 1. Check Drivers (Main DB)
      const { data: driver } = await supabaseZaptro
        .from('whatsapp_drivers')
        .select('*, whatsapp_vehicles(*)')
        .eq('company_id', profile.company_id)
        .eq('phone', digits)
        .maybeSingle();

      if (driver) {
        const result = { type: 'FROTA' as const, data: driver, name: driver.full_name || driver.name || 'Motorista' };
        setContactClassification(result.type);
        setContactExtendedData(result.data);
        setCurrentSectorLayout('LOGISTICA');
        return result;
      }

      // 2. Check Profiles / Staff (Main DB)
      const { data: staff } = await supabaseZaptro
        .from('profiles')
        .select('*')
        .eq('company_id', profile.company_id)
        .or(`metadata->>phone.eq.${digits},email.ilike.%${digits}%`)
        .maybeSingle();
      
      if (staff) {
        const result = { type: 'COLABORADOR' as const, data: staff, name: staff.full_name || 'Colaborador' };
        setContactClassification(result.type);
        setContactExtendedData(result.data);
        setCurrentSectorLayout('SUPORTE');
        return result;
      }

      // 3. Check Leads (Hub DB)
      const { data: lead } = await hubSupabase
        .from('leads')
        .select('*')
        .eq('phone', digits)
        .maybeSingle();
      ...
      if (lead) {
        const result = { type: 'LEAD' as const, data: lead, name: lead.name || 'Lead' };
        setContactClassification(result.type);
        setContactExtendedData(result.data);
        setCurrentSectorLayout('COMERCIAL');
        return result;
      }

      // 4. Check Clients (Hub DB - Companies)
      const { data: client } = await hubSupabase
        .from('companies')
        .select('*')
        .or(`metadata->>phone.eq.${digits},document.eq.${digits}`)
        .maybeSingle();

      if (client) {
        const result = { type: 'CLIENTE' as const, data: client, name: client.name || 'Cliente' };
        setContactClassification(result.type);
        setContactExtendedData(result.data);
        setCurrentSectorLayout('FINANCEIRO');
        return result;
      }

      setContactClassification('NOVO');
      setContactExtendedData(null);
      return { type: 'NOVO' as const, data: null, name: phone };
    } catch (err) {
  ...
  useEffect(() => {
    if (selectedChat?.sender_number) {
      identifyContact(selectedChat.sender_number);
    } else {
      setContactClassification(null);
      setContactExtendedData(null);
    }
  }, [selectedChat]);
  ...
  const handleScheduleFollowup = async () => {
    ...
    if (!followupForm.title.trim() || !followupForm.dueAt) {
      notifyZaptro('info', 'Follow-up', 'Preencha o título e a data/hora.');
      return;
    }

    setScheduling(true);
    try {
      const leadId = contactExtendedData?.id || selectedChat.id;

      await insertCrmTask({
        companyId: profile.company_id,
        leadId: leadId,
        title: followupForm.title,
        dueAtIso: new Date(followupForm.dueAt).toISOString(),
        notes: followupForm.notes,
        createdBy: profile.id,
      });

      notifyZaptro('success', 'Follow-up', 'Compromisso agendado com sucesso no CRM.');
      setFollowupModalOpen(false);
    } catch (err: any) {
      console.error('Error scheduling followup:', err);
      if (err.message === 'MISSING_TABLE') {
        notifyZaptro('warning', 'Follow-up', 'Tabela de tarefas não encontrada na base. Agendamento simulado.');
        setFollowupModalOpen(false);
      } else {
        toastError('Não foi possível agendar o follow-up.');
      }
    } finally {
      setScheduling(false);
    }
  ...
  useEffect(() => {
    ...
    // No longer auto-selecting demo chat
  ...
  }, [waThread, loading, conversations, mergedDemoConversations, profile?.company_id]);
    ...
    const fetchFlow = async () => {
      try {
        const { data, error } = await supabaseZaptro
          ...
          .limit(1)
          .maybeSingle();
        if (data) setAutomationFlow(data);
      } catch (err) {
  ...
  const processAutomationFlow = useCallback(async (flow: any, messageRow: any, conversationId: string) => {
    const text = (messageRow.content || '').toString().trim().toLowerCase();
    const companyId = profile?.company_id;
    ...
    const match = options.find((opt: any, idx: number) => {
      const trigger = (opt.trigger || '').toString().toLowerCase();
      const optionIndex = (idx + 1).toString();
      return text === trigger || text === optionIndex;
    });

    if (match) {
      if (match.action === 'routing' && match.department) {
        await supabaseZaptro
          .from('whatsapp_conversations')
          .update({ 
            department: match.department.toUpperCase(), 
            attendance_status: 'awaiting',
            assigned_to: null // Volta para fila
          })
      ...
      if (flow.welcome_message) {
        let fullMsg = flow.welcome_message;
        if (options.length > 0) {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (dir === 'in') {
            ...
            const avatarUrl = hit?.metadata?.avatar_url || null;
            ...
            showWaDesktopNotificationIfAllowed({
              ...
              avatarUrl,
              onClick: () => {
                navigate(zaptroWhatsappInboxThreadPath(convId));
              }
            });
          }

          if (dir === 'in' && automationFlow) {
  ...
  const handleMarkAsClient = async () => {
    if (!selectedChat || converting) return;
    setConverting(true);
    try {
      // 1. Update locally in Zaptro
      const { error: zError } = await supabaseZaptro
        .from('whatsapp_conversations')
      ...
      const clientName = selectedChat.sender_name || selectedChat.sender_number || 'Novo Cliente';
      const clientSlug = (selectedChat.sender_name || selectedChat.sender_number)
        .toLowerCase()
        .replace(/[^a-z0-9]/g, '-')
        .replace(/-+/g, '-')
        .substring(0, 50);
      ...
      const { error: hError } = await hubSupabase
        ...
        .upsert({
          name: clientName,
          slug: clientSlug,
          document: selectedChat.sender_number, // Fallback for identification
          status: 'active',
          origin: 'zaptro',
          metadata: {
            zaptro_conversation_id: selectedChat.id,
            synced_at: new Date().toISOString()
          }
        }, { onConflict: 'document' });

      if (hError) {
        console.error('Hub Sync Error:', hError);
        notifyZaptro('warning', 'Sincronização Hub', 'Cliente salvo localmente, mas houve um erro ao replicar para o Master Hub.');
      } else {
        notifyZaptro('success', 'CRM', 'Cliente convertido e sincronizado com o Hub com sucesso!');
      }

      // Refresh list
      void fetchConversationsFromDb({ manageLoading: false });
      setSelectedChat(prev => prev ? { ...prev, crm_type: 'client' } : null);
    } catch (err) {
      console.error('Conversion error:', err);
      notifyZaptro('error', 'Erro', 'Não foi possível converter o lead em cliente.');
    } finally {
      setConverting(false);
    }
  };
  ...
  const handleQuickCall = async () => {
    if (!quickCallNumber || quickCallChecking) return;
    const cleanNumber = quickCallNumber.replace(/\D/g, '');
    if (cleanNumber.length < 8) {
      notifyZaptro('warning', 'Número inválido', 'Por favor insira um número válido (ex: 5511...)');
      return;
    }

    setQuickCallChecking(true);
    setQuickCallError(null);

    // 1. Intelligent Identification
    const identity = await identifyContact(cleanNumber);

    // 2. Real WhatsApp Network Check
    try {
      ...
      // Fallback: alert but let them try
      notifyZaptro('warning', 'WhatsApp', 'Não foi possível verificar o número, mas tentaremos abrir a conversa.');
    }

    // 3. Check if conversation already exists in current list
    const existing = conversations.find(c => c.sender_number === cleanNumber);
    if (existing) {
      setQuickCallChecking(false);
      setQuickCallModalOpen(false);
      setQuickCallNumber('');
    ...
    // Cria o chat virtual localmente
    const virtualChat: Conversation = {
  ...
  const handleSendAutomationMenu = async () => {
    if (!selectedChat || !profile?.company_id) return;
  ...
  return (
    ...
    <div style={shellStyle}>
      ...
      <div style={{ ...styles.container, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ ...styles.sidebar, borderRight: `1px solid ${palette.sidebarBorder}`, backgroundColor: palette.sidebarBg, position: 'relative' }}>
            ...
            return (
              <button
                ...
                style={{
                  padding: '8px 20px',
                  borderRadius: 16,
                  border: on ? 'none' : `1px solid ${palette.sidebarBorder}`,
        ...
        <div style={{ ...styles.chatList }}>
          {loading ? (
            ...
            <div style={{ padding: 60, textAlign: 'center', color: palette.textMuted }}>
              <div style={{ 
                width: 80, 
                height: 80, 
                borderRadius: 24, 
                backgroundColor: palette.mode === 'dark' ? 'rgba(255,255,255,0.03)' : '#f4f4f5',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                margin: '0 auto 20px',
              ...
              <p style={{ fontSize: 16, color: palette.text, fontWeight: 800, margin: 0 }}>Nenhuma conversa ativa</p>
              <p style={{ fontSize: 13, color: palette.textMuted, fontWeight: 600, marginTop: 8, lineHeight: 1.5 }}>
        ...
        {selectedChat ? (
          ...
          >
            {/* CHAT HEADER */}
            <div style={{ ...styles.chatHeader, borderBottom: `1px solid ${palette.sidebarBorder}`, backgroundColor: palette.sidebarBg }}>
              <div 
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 1, height: 16, backgroundColor: palette.sidebarBorder, margin: '0 4px' }} />
                      <span style={{ 
                        fontSize: 10, 
                        fontWeight: 800, 
                        color: SECTOR_LABELS[currentSectorLayout].color,
                        display: 'flex',
                        alignItems: 'center',
                        gap: 4,
                        textTransform: 'uppercase'
                      }}>
                        {React.createElement(SECTOR_LABELS[currentSectorLayout].icon, { size: 10 })}
                        {SECTOR_LABELS[currentSectorLayout].label}
                      </span>
                    </div>
                  </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {searchInsideOpen ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <button
                  type="button"
                  title="Enviar Menu de Automação"
                  style={{ 
                    width: 42, 
                    height: 42, 
                    borderRadius: 14, 
                  ...
                  onClick={handleSendAutomationMenu}
                >
                  <Zap size={18} fill={LIME} />
                </button>
                <button
                  type="button"
                  style={{ 
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 600, lineHeight: 1.4 }}>{msg.content}</p>
            ...
            {(!sendBlocked || isAdmin) && (
              <div style={{ padding: '12px 24px', backgroundColor: palette.sidebarBg, borderTop: `1px solid ${palette.sidebarBorder}` }}>
                {sendBlocked && isAdmin && (
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const chips = [
                      ...(currentSectorLayout === 'LOGISTICA' ? [
                        { label: 'Nova Carga', icon: Truck, color: LIME, action: () => navigate(ZAPTRO_ROUTES.ROUTES) },
                        { label: 'Ocorrência', icon: AlertCircle, color: '#ef4444', action: () => notifyZaptro('info', 'Ocorrência', 'Abrindo formulário...') },
                        { label: 'Rastreio', icon: MapPin, color: '#3b82f6', action: () => {
                          if (selectedChat?.sender_number) {
                            setNewMessage(`Olá! Você pode acompanhar seu pedido em tempo real através deste link: https://zaptro.com/t/${onlyDigits(selectedChat.sender_number)}`);
                          }
                        }}
                      ] : []),
                      ...(currentSectorLayout === 'FINANCEIRO' ? [
                        { label: 'Solicitar Pix', icon: Zap, color: '#f59e0b', action: () => {
                          const s = company?.settings as any;
                          const pix = s?.pix_key;
                          if (pix) {
                            setNewMessage(`Seguem os dados para pagamento via Pix:\n\n🔑 Chave Pix: ${pix}\n🏦 Banco: ${s?.bank_name || '—'}\n👤 Titular: ${s?.bank_holder || company?.name || '—'}`);
                            notifyZaptro('success', 'Pix', 'Dados inseridos.');
                          } else {
                            notifyZaptro('warning', 'Pix não configurado', 'Cadastre sua chave em Empresa > Configurações.');
                          }
                        }},
                        { label: 'Cobrança', icon: CreditCard, color: '#3b82f6', action: () => setBillingModalOpen(true) },
                        { label: 'Faturas', icon: FileText, color: '#10b981', action: () => notifyZaptro('info', 'Financeiro', 'Consultando faturas...') }
                      ] : []),
                      ...(currentSectorLayout === 'COMERCIAL' ? [
                        { label: 'Orçamento', icon: FileText, color: '#3b82f6', action: () => setQuotesModalOpen(true) },
                        { label: 'Proposta', icon: Sparkles, color: '#a855f7', action: () => notifyZaptro('info', 'Comercial', 'Enviando proposta...') },
                        { label: 'Vincular Lead', icon: UserPlus, color: LIME, action: () => setContactsModalOpen(true) }
                      ] : []),
                      ...(currentSectorLayout === 'SUPORTE' ? [
                        { label: 'FAQ', icon: Info, color: '#3b82f6', action: () => notifyZaptro('info', 'Suporte', 'Enviando respostas frequentes...') },
                        { label: 'Ticket', icon: AlertCircle, color: '#f59e0b', action: () => notifyZaptro('info', 'Suporte', 'Ticket aberto para esta conversa.') },
                        { label: 'Transferir', icon: Share, color: '#a855f7', action: () => notifyZaptro('info', 'Suporte', 'Selecione o colega para transferir...') }
                      ] : []),
                      { label: 'Vincular Equipe', icon: User, color: '#fff', action: () => setContactsModalOpen(true) },
                    ];
                    ...
                    return chips.map((chip, i) => (
                      <button
                        ...
                        style={{
                          ...
                          padding: '6px 14px',
                          borderRadius: 14,
                          backgroundColor: palette.mode === 'dark' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)',
                          border: `1px solid ${palette.mode === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)'}`,
                          color: palette.text,
                          fontSize: 11,
                          fontWeight: 800,
                          whiteSpace: 'nowrap',
                        ...
                        onMouseEnter={(e) => {
                          e.currentTarget.style.backgroundColor = palette.mode === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)';
                          e.currentTarget.style.transform = 'translateY(-2px)';
                          e.currentTarget.style.borderColor = chip.color;
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.backgroundColor = palette.mode === 'dark' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)';
                          e.currentTarget.style.transform = 'translateY(0)';
                          e.currentTarget.style.borderColor = palette.mode === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)';
                        }}
                      >
                ...
                >
                  <div style={{ position: 'relative' }}>
                    ...
                    <button 
                      type="button" 
                      disabled={isBillingBlocked || (sendBlocked && !isAdmin)}
                      style={{ ...styles.inputSideBtn, color: palette.textMuted, opacity: isBillingBlocked || (sendBlocked && !isAdmin) ? 0.5 : 1, cursor: isBillingBlocked || (sendBlocked && !isAdmin) ? 'not-allowed' : 'pointer' }} 
                  ...
                  <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    {isRecording ? (
                      ...
                      <textarea
                        ...
                        placeholder={
                          isBillingBlocked
                            ? "Plano Expirado/Sem Créditos. Vá para Faturamento."
                        ...
                        disabled={isBillingBlocked || (sendBlocked && !isAdmin) || sending}
                        style={{
                          width: '100%',
                          border: 'none',
                          background: 'transparent',
                  ...
                  {!newMessage.trim() && !isRecording ? (
                    <button
                      ...
                      style={{
                        ...
                        justifyContent: 'center',
                        border: 'none',
                        cursor: (isBillingBlocked || (sendBlocked && !isAdmin)) ? 'not-allowed' : 'pointer',
                        color: palette.textMuted,
                        transition: 'transform 0.2s ease',
                        opacity: (isBillingBlocked || (sendBlocked && !isAdmin)) ? 0.5 : 1
                      }}
                      onMouseEnter={(e) => { if (!isBillingBlocked && !(sendBlocked && !isAdmin)) e.currentTarget.style.transform = 'scale(1.1)'; }}
                      onMouseLeave={(e) => { if (!isBillingBlocked && !(sendBlocked && !isAdmin)) e.currentTarget.style.transform = 'scale(1)'; }}
                    >
                    ...
                    <button
                      type="submit"
                      disabled={isBillingBlocked || (sending || (sendBlocked && !isAdmin)) && !isRecording}
      ...
      {showRightPanel && selectedChat && (
        <div style={{ 
          ...
          {/* RIGHT HEADER */}
          <div style={{ ...styles.rightHead, borderBottom: `1px solid ${palette.sidebarBorder}`, position: 'relative' }}>
             <button 
              ...
              <p style={{ margin: "4px 0 0", fontSize: 14, color: palette.textMuted, fontWeight: 600, textAlign: "center" }}>{selectedChat.sender_number || selectedChat.sender_name}</p>
               ...
               {classificationLoading ? (
                 <div style={{ textAlign: 'center', marginTop: 8 }}>
                   <Zap size={14} className="zaptro-spin" color={LIME} />
                   <span style={{ fontSize: 10, fontWeight: 800, color: LIME, marginLeft: 6, textTransform: 'uppercase' }}>Identificando contato...</span>
                 </div>
                 ...
                 <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10, gap: 8 }}>
                    <div style={{
                      ...
                      padding: '4px 10px',
                      borderRadius: 20,
                      backgroundColor: contactClassification === 'FROTA' ? LIME : 'rgba(217, 255, 0, 0.1)',
                      color: contactClassification === 'FROTA' ? '#000' : LIME,
                      border: `1px solid ${LIME}`,
                      textTransform: 'uppercase',
                    ...
                    <div style={{
                      ...
                      {(() => {
                        const s = SECTOR_LABELS[currentSectorLayout];
                        const Icon = s.icon;
                        return <><Icon size={12} color={s.color} /> {s.label}</>;
                      })()}
                    </div>
                 </div>
               )}
              
               {(contactClassification === 'LEAD' || contactClassification === 'NOVO') && (
                <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: '0 8px 20px rgba(217, 255, 0, 0.25)',
                      transition: 'all 0.2s ease',
                      transform: converting ? 'scale(0.98)' : 'scale(1)'
                    }}
                  >
                    {converting ? (
                      <Zap size={16} className="zaptro-spin" />
                    ) : (
                      <UserPlus size={18} />
                    )}
              ...
              {contactClassification === 'CLIENTE' && (
                <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
                  <div style={{
                    ...
                    <Globe size={14} /> Cliente Sincronizado Hub
                  </div>
                </div>
              )}
           </div>
          {/* DYNAMIC SECTIONS BASED ON CONTACT TYPE / SECTOR */}
          <div style={{ ...styles.rightSection, borderBottom: `1px solid ${palette.sidebarBorder}` }}>
             <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <h4 style={{ ...styles.rightH4, margin: 0 }}>
                  {contactClassification === 'FROTA' ? 'PAINEL DO MOTORISTA' : 
                   contactClassification === 'COLABORADOR' ? 'PERFIL COLABORADOR' :
                   contactClassification === 'LEAD' ? 'GESTÃO DE LEAD' :
                   contactClassification === 'CLIENTE' ? 'PORTAL DO CLIENTE' : 'INFORMAÇÕES ADICIONAIS'}
                </h4>
                <div style={{ display: 'flex', gap: 4 }}>
                  {(['LOGISTICA', 'FINANCEIRO', 'COMERCIAL', 'SUPORTE'] as const).map(s => (
                    <button 
                      key={s}
                      onClick={() => setCurrentSectorLayout(s)}
                      title={s}
                      style={{
                        width: 24,
                        height: 24,
                        borderRadius: 6,
                        border: 'none',
                        backgroundColor: currentSectorLayout === s ? SECTOR_LABELS[s].color : 'transparent',
                        color: currentSectorLayout === s ? '#000' : palette.textMuted,
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        cursor: 'pointer',
                    ...
                    >
                      {React.createElement(SECTOR_LABELS[s].icon, { size: 12 })}
                    </button>
                  ))}
                </div>
             </div>

             {contactClassification === 'FROTA' && (
               <div style={{ marginTop: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                     <span style={{ fontSize: 12, fontWeight: 700, color: palette.text }}>Progresso da Rota</span>
                     <span style={{ fontSize: 12, fontWeight: 700, color: LIME }}>82%</span>
                  </div>
                  <div style={{ height: 6, width: '100%', backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: 3, overflow: 'hidden' }}>
                     <div style={{ height: '100%', width: '82%', backgroundColor: LIME, borderRadius: 3 }} />
                  </div>
                  
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: 'rgba(217, 255, 0, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Truck size={16} color={LIME} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <p style={{ margin: 0, fontSize: 10, fontWeight: 600, color: palette.textMuted, textTransform: 'uppercase' }}>Veículo Atual</p>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: palette.text }}>
                          {contactExtendedData?.vehicle || contactExtendedData?.whatsapp_vehicles?.[0]?.plate || 'Scania R450 (ABC-1234)'}
                        </p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: 'rgba(59, 130, 246, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <MapPin size={16} color="#3b82f6" />
                      </div>
                      <div style={{ flex: 1 }}>
                        <p style={{ margin: 0, fontSize: 10, fontWeight: 600, color: palette.textMuted, textTransform: 'uppercase' }}>Última Parada</p>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: palette.text }}>Posto Graal, BR-116</p>
                      </div>
                    </div>
                  </div>
               </div>
             ...
             {contactClassification === 'LEAD' && (
               <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                 <div style={{ padding: 12, borderRadius: 12, backgroundColor: 'rgba(168, 85, 247, 0.05)', border: '1px solid rgba(168, 85, 247, 0.1)' }}>
                   <p style={{ margin: '0 0 4px', fontSize: 10, fontWeight: 800, color: '#a855f7', textTransform: 'uppercase' }}>Temperatura</p>
                   <div style={{ display: 'flex', gap: 4 }}>
                     {['FRIO', 'MORNO', 'QUENTE'].map(t => (
                       <div key={t} style={{ 
                         flex: 1, height: 4, borderRadius: 2, 
                         backgroundColor: contactExtendedData?.temperature?.toUpperCase() === t ? (t === 'QUENTE' ? '#ef4444' : t === 'MORNO' ? '#f59e0b' : '#3b82f6') : 'rgba(0,0,0,0.05)'
                       }} />
                     ))}
                   </div>
                   <p style={{ margin: '8px 0 0', fontSize: 13, fontWeight: 700, color: palette.text }}>
                     {contactExtendedData?.temperature?.toUpperCase() || 'MORNO'}
                   </p>
                 </div>
                 ...
                 <div style={{ display: 'flex', gap: 8 }}>
                   <button 
                     onClick={() => {
                       setFollowupForm({ 
                         title: `Call de acompanhamento: ${selectedChat?.sender_name || selectedChat?.sender_number}`, 
                         dueAt: new Date(Date.now() + 86400000).toISOString().slice(0, 16), 
                         notes: '' 
                       });
                       setFollowupModalOpen(true);
                     }}
                     style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1px solid ${palette.sidebarBorder}`, background: 'transparent', color: palette.text, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
                   >
                     Agendar Call
                   </button>
                   <button 
                     onClick={() => {
                       setFollowupForm({ 
                         title: `Enviar proposta para ${selectedChat?.sender_name || selectedChat?.sender_number}`, 
             ...
             {contactClassification === 'COLABORADOR' && (
               <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                 <div style={{ padding: 12, borderRadius: 12, backgroundColor: 'rgba(0,0,0,0.03)' }}>
                    <p style={{ margin: '0 0 4px', fontSize: 10, fontWeight: 800, color: palette.textMuted, textTransform: 'uppercase' }}>Departamento</p>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: palette.text }}>{contactExtendedData?.department || 'Operacional'}</p>
                 </div>
                 <div style={{ padding: 12, borderRadius: 12, backgroundColor: 'rgba(0,0,0,0.03)' }}>
                    <p style={{ margin: '0 0 4px', fontSize: 10, fontWeight: 800, color: palette.textMuted, textTransform: 'uppercase' }}>Cargo</p>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: palette.text }}>{contactExtendedData?.role || 'Analista'}</p>
                 </div>
               </div>
             )}
             ...
             {contactClassification === 'CLIENTE' && (
               <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                 <div style={{ padding: 14, borderRadius: 16, backgroundColor: 'rgba(34, 197, 94, 0.05)', border: '1px solid rgba(34, 197, 94, 0.1)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <p style={{ margin: 0, fontSize: 10, fontWeight: 800, color: '#22c55e', textTransform: 'uppercase' }}>Plano Ativo</p>
                      <CheckCircle size={14} color="#22c55e" />
                    </div>
                    <p style={{ margin: 0, fontSize: 16, fontWeight: 800, color: palette.text }}>Zaptro Enterprise</p>
                    <p style={{ margin: '4px 0 0', fontSize: 12, fontWeight: 600, color: palette.textMuted }}>Próxima fatura: 12/05</p>
                 </div>
                 
                 <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <button style={{ width: '100%', padding: '10px', borderRadius: 10, border: `1px solid ${palette.sidebarBorder}`, background: 'transparent', color: palette.text, fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <FileText size={14} /> Histórico de Faturas
                    </button>
                    <button style={{ width: '100%', padding: '10px', borderRadius: 10, border: `1px solid ${palette.sidebarBorder}`, background: 'transparent', color: palette.text, fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Zap size={14} /> Logs de Integração
                    </button>
                 </div>
               </div>
             )}

             {contactClassification === 'NOVO' && !classificationLoading && (
               <div style={{ textAlign: 'center', padding: '20px 0' }}>
                 <div style={{ width: 48, height: 48, borderRadius: '50%', backgroundColor: 'rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                    <UserPlus size={24} color={palette.textMuted} />
                 </div>
                 <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: palette.text }}>Contato não identificado</p>
                 <p style={{ margin: '4px 0 0', fontSize: 12, color: palette.textMuted, fontWeight: 600 }}>Cadastre como LEAD ou FROTA para habilitar ferramentas inteligentes.</p>
               </div>
             )}
             ...
             {selectedChat.assigned_to && (
               <div style={{ 
                 ...
                 <div style={{ flex: 1 }}>
                   <p style={{ margin: 0, fontSize: 10, fontWeight: 800, color: LIME, textTransform: 'uppercase' }}>Atendente Zaptro</p>
                   <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: palette.text }}>
          ...
          <div style={{ ...styles.rightSection, borderBottom: `1px solid ${palette.sidebarBorder}`, padding: "16px 20px" }}>
             <h4 style={styles.rightH4}>RASTREAMENTO OPERACIONAL</h4>
             <div style={{ borderRadius: 16, overflow: "hidden", border: `1px solid ${palette.sidebarBorder}`, marginTop: 12 }}>
                <OpenStreetRouteMap 
                  height="240px" 
                  mode="tracking"
    ...
    {callActionModal && selectedChat ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            ...
            <button
              ...
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = palette.sidebarBorder)}
            >
      ...
      <LogtaModal
        ...
        title="Agendar Compromisso Comercial"
        width="480px"
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '10px 0' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ fontSize: 11, fontWeight: 800, color: palette.textMuted, textTransform: 'uppercase' }}>Título / Objetivo</label>
            <input
              style={{
                padding: '12px 16px',
                borderRadius: 12,
              ...
              placeholder="Ex: Call de fechamento..."
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ fontSize: 11, fontWeight: 800, color: palette.textMuted, textTransform: 'uppercase' }}>Data e Hora Agendada</label>
            <input
              type="datetime-local"
              style={{
                padding: '12px 16px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ fontSize: 11, fontWeight: 800, color: palette.textMuted, textTransform: 'uppercase' }}>Notas / Briefing</label>
            <textarea
              style={{
                padding: '12px 16px',
                borderRadius: 12,
              ...
              placeholder="Descreva o que foi conversado ou o objetivo da call..."
      ...
      {quickCallModalOpen && (
        <div style={{
          ...
          <div style={{
            ...
            animation: 'zaptroFadeIn 0.3s ease-out'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
              <div style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: LIME, display: 'flex', alignItems: 'center', justifyCenter: 'center', flexShrink: 0 }}>
                <Phone size={22} color="#000" />
              </div>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: palette.text }}>Chamar número</h2>
            </div>

            <p style={{ fontSize: 14, color: palette.textMuted, marginBottom: 24, lineHeight: 1.5 }}>
              Insira o número de WhatsApp que deseja iniciar a conversa. Use o formato com DDD (ex: 5511...).
            </p>
            ...
            <div style={{ marginBottom: 24 }}>
              <input
                ...
                disabled={quickCallChecking}
                onChange={(e) => {
                  setQuickCallNumber(e.target.value);
                  setQuickCallError(null);
                }}
                autoFocus
                ...
                style={{
                  width: '100%',
                  backgroundColor: palette.mode === 'dark' ? '#202124' : '#f1f5f9',
                  border: `1px solid ${quickCallError ? '#ef4444' : palette.sidebarBorder}`,
                  borderRadius: 14,
                  padding: '16px 20px',
                  fontSize: 18,
                  fontWeight: 700,
                  ...
                  textAlign: 'center',
                  letterSpacing: '0.05em',
                  opacity: quickCallChecking ? 0.6 : 1
                }}
              />
              {quickCallError && (
                <p style={{ color: '#ef4444', fontSize: 13, fontWeight: 700, margin: '12px 0 0', textAlign: 'center' }}>
                  {quickCallError}
                </p>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button
                onClick={() => setQuickCallModalOpen(false)}
                disabled={quickCallChecking}
                style={{
                  flex: 1,
                  padding: '16px',
                  ...
                  color: palette.text,
                  fontWeight: 700,
                  cursor: quickCallChecking ? 'not-allowed' : 'pointer',
                  opacity: quickCallChecking ? 0.5 : 1
                }}
              >
                Cancelar
              </button>
              <button
                ...
                style={{
                  ...
                  borderRadius: 14,
                  border: 'none',
                  backgroundColor: quickCallChecking ? '#e2e8f0' : LIME,
                  color: '#000',
                  fontWeight: 800,
                  cursor: quickCallChecking ? 'wait' : 'pointer',
                  boxShadow: quickCallChecking ? 'none' : `0 8px 20px ${LIME}44`,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  gap: 8
              ...
              >
                {quickCallChecking ? (
                  <>
                    <div className="zaptro-spinner-small" /> Verificando...
                  </>
                ) : 'Conversar'}
              </button>
            </div>
