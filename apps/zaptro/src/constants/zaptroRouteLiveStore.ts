...
export type RouteLiveBucket = {
  ...
  /** Data em que a rota foi concluída (status 'delivered'). */
...
export function readAllRouteLive(): Record<string, RouteLiveBucket> {
  try {
    ...
    // Auto-cleanup: remove tokens finished > 24h ago
    let changed = false;
    const now = Date.now();
    const twentyFourHours = 24 * 60 * 60 * 1000;
    const clean: Record<string, RouteLiveBucket> = {};

    for (const token in p) {
      const bucket = p[token];
      if (bucket.status === 'delivered' && bucket.finishedAt) {
        const finished = new Date(bucket.finishedAt).getTime();
        if (now - finished > twentyFourHours) {
          changed = true;
          continue;
        }
      }
...
export function readRouteLive(token: string): RouteLiveBucket | null {
  ...
  const bucket = readAllRouteLive()[t];
  if (!bucket) return null;

  // Expiração: 24h após 'delivered'
  if (bucket.status === 'delivered' && bucket.finishedAt) {
    const finished = new Date(bucket.finishedAt).getTime();
    const now = Date.now();
    const twentyFourHours = 24 * 60 * 60 * 1000;
...
export function patchRouteLive(token: string, patch: Partial<RouteLiveBucket>): RouteLiveBucket {
  ...
  // Regista quando a rota é finalizada
  if (patch.status === 'delivered' && !prev.finishedAt) {
    next.finishedAt = new Date().toISOString();
  } else if (patch.status && patch.status !== 'delivered') {
    // Se voltar atrás (ex: reabrir), remove a data de finalização
