  ...
  return (
    <header className="h-20 bg-transparent flex items-center justify-between px-8 z-50">
      <div className="flex items-center gap-4">
        {config.logoUrl ? (
          <img src={config.logoUrl} alt="" className="h-12 w-auto max-w-[180px] object-contain" aria-label="Logo da empresa" />
        ) : (
          <svg className="h-10 w-10 shrink-0 text-primary" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Logta">
            <path d="M10 10 V90 H90 V60 H40 V10 Z" fill="currentColor" />
            <rect x="50" y="10" width="40" height="40" fill="currentColor" />
          </svg>
        )}
      </div>
      ...
      <main className="flex-1 flex flex-col h-screen min-w-0">
        ...
        <div className="flex min-h-0 flex-1 flex-col px-8 pb-8 pt-4">
          ...
          <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[40px] border border-gray-100 bg-white shadow-sm">
            ...
            <Suspense fallback={<AuthPageFallback message="Carregando módulo…" />}>
            <Routes>
              ...
              <Route path="/fiscal/*" element={<Documentos />} />
