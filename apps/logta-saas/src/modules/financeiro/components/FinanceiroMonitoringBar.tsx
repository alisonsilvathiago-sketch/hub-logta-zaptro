...
export function FinanceiroMonitoringBar() {
    ...
    return (
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-green-200 bg-green-50/80 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-bold text-green-800">Monitoramento financeiro ativo — operação estável</span>
        </div>
  ...
  return (
    <div className={`flex flex-wrap items-center justify-between gap-3 rounded-2xl border px-4 py-3 ${nivelColor}`}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs font-bold text-gray-900">
          {monitoring.critical > 0
            ? `${monitoring.critical} alerta(s) crítico(s) · ${monitoring.total} no total`
            : `${monitoring.total} alerta(s) financeiro(s) detectado(s)`}
        </span>
        {lastScanAt ? (
          <span className="text-[10px] font-medium text-gray-500">
            Atualizado {lastScanAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
          </span>
        ) : null}
      </div>
