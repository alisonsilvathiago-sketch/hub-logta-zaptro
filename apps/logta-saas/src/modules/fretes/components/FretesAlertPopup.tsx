...
import { AlertTriangle, ArrowRight, Bell, MapPin, Sparkles, Truck, X } from 'lucide-react';
...
export function FretesAlertPopup({ alert, onClose, onDismiss, queueCount = 0 }: Props) {
  const navigate = useNavigate();
  const s = styles[alert.priority];

  const Icon =
    alert.category === 'rota' ? MapPin : alert.category === 'motorista' ? Truck : alert.category === 'ia' ? Sparkles : AlertTriangle;
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      >
        ...
        {(alert.impacto || alert.recomendacao) && (
