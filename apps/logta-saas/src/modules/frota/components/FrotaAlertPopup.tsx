...
export function FrotaAlertPopup({ alert, onClose, onDismiss }: Props) {
  const navigate = useNavigate();
  const s = styles[alert.priority];
  const Icon =
    alert.category === 'combustivel'
      ? Fuel
      : alert.category === 'manutencao'
        ? Wrench
        : alert.category === 'ia'
          ? Sparkles
          : alert.category === 'documento'
            ? FileText
            : alert.category === 'ipva' || alert.category === 'licenciamento'
              ? FileText
              : Truck;
      ...
      >
        ...
        {alert.impacto ? (
          <div className="mb-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-neutral-400">
            <strong className="text-white">Impacto:</strong> {alert.impacto}
          </div>
        ) : null}
