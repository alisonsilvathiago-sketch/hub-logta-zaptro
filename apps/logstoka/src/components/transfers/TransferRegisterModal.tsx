...
import { ArrowLeftRight, MapPin, Minus, Package, Plus, ShieldCheck } from 'lucide-react';
...
import TransferAuthorizationTerm from '@/components/transfers/TransferAuthorizationTerm';
import './transferRegisterModal.css';
...
const TransferRegisterModal: React.FC<Props> = ({
  ...
  const originWh = warehouses.find((w) => w.id === form.origin);
  const destWh = warehouses.find((w) => w.id === form.destination);
  const product = scan.resolvedProduct;

  useEffect(() => {
    if (!open || !originWh?.manager_name || form.releasedByName.trim()) return;
    onChange({ releasedByName: originWh.manager_name });
  }, [open, originWh?.manager_name, originWh?.id, form.releasedByName, onChange]);
  ...
  const stockAtOrigin = useMemo(() => {
    if (!product || !form.origin || !scan.companyId) return 0;
    return getProductStockAtWarehouse(product.id, form.origin, scan.companyId);
  }, [product, form.origin, scan.companyId]);
  ...
  const footer = (
    <>
      ...
      <button type="button" className="ls-btn-primary" disabled={saving} onClick={onSubmit}>
        {saving ? 'Registrando…' : 'Aprovar e registrar'}
      </button>
    </>
  );
  ...
  return (
    <Modal
      ...
      subtitle="Busque o produto pelo código LS ou EAN"
      icon={<ArrowLeftRight size={20} strokeWidth={2.25} />}
      onClose={onClose}
      footer={footer}
    >
      <div className="ls-transfer-modal">
        <div className="ls-transfer-modal__route">
          <div className="ls-transfer-modal__route-field">
            ...
            >
              <option value="">CD de origem</option>
              {warehouses.map((w) => (
                <option key={w.id} value={w.id} disabled={w.id === form.destination}>
                  {w.name}
                  {w.city ? ` · ${w.city}` : ''}
                </option>
              ))}
          ...
          <span className="ls-transfer-modal__route-arrow" aria-hidden>
          ...
          <div className="ls-transfer-modal__route-field">
            ...
            >
              <option value="">CD de destino</option>
              {warehouses.map((w) => (
                <option key={w.id} value={w.id} disabled={w.id === form.origin}>
                  {w.name}
                  {w.city ? ` · ${w.city}` : ''}
                </option>
              ))}
        ...
        <div className="ls-transfer-modal__main">
          <div className="ls-transfer-modal__scan-wrap">
            <MovementScanSection
              ...
              inputId="ls-transfer-scan-input"
            />
          </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {form.origin ? (
                    <p className="ls-transfer-modal__product-stock">
                      Saldo origem: <strong>{stockAtOrigin.toLocaleString('pt-BR')}</strong>
                    </p>
                  ) : null}
                </div>
              </div>
        ...
        <section className="ls-transfer-modal__approval">
          ...
          <div className="ls-transfer-modal__approval-grid">
            <div className="ls-transfer-modal__approval-field">
              ...
              <input
                className="ls-input"
                placeholder="Quem libera no CD origem"
          ...
          <label className="ls-transfer-modal__confirm">
            ...
            />
            <span>
              Li o termo acima, conferi itens e quantidades e autorizo a transferência de{' '}
              <strong>{originWh?.name ?? 'origem'}</strong> para <strong>{destWh?.name ?? 'destino'}</strong>.
            </span>
          </label>
        </section>
      </div>
