      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '32px' }}>
           <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div style={{ padding: '24px', backgroundColor: '#F8FAFC', borderRadius: '20px', border: '1px solid var(--border)' }}>
                 <h5 style={{ margin: '0 0 16px 0', fontSize: '11px', fontWeight: '600', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>ESTATÍSTICAS DO CLUSTER</h5>
                 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                    <div style={{ textAlign: 'center' }}>
                       <div style={{ fontSize: '11px', fontWeight: '600', color: '#94A3B8', letterSpacing: '0.4px' }}>CPU</div>
                       <div style={{ fontSize: '20px', fontWeight: '500', color: 'var(--text-main)' }}>{selectedInstance?.cpu}</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                       <div style={{ fontSize: '11px', fontWeight: '600', color: '#94A3B8', letterSpacing: '0.4px' }}>MEMÓRIA</div>
                       <div style={{ fontSize: '20px', fontWeight: '500', color: 'var(--text-main)' }}>{selectedInstance?.memory}</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                       <div style={{ fontSize: '11px', fontWeight: '600', color: '#94A3B8', letterSpacing: '0.4px' }}>UPTIME</div>
                       <div style={{ fontSize: '20px', fontWeight: '500', color: 'var(--text-main)' }}>{selectedInstance?.uptime}</div>
                    </div>
                 </div>
              </div>

              <div style={{ padding: '24px', backgroundColor: '#0F172A', borderRadius: '20px', color: '#4ADE80', fontFamily: 'monospace', minHeight: '200px' }}>
                 <div style={{ fontSize: '10px', color: '#64748B', marginBottom: '12px', fontWeight: 'bold', letterSpacing: '0.8px' }}>LOGS DO ENGINE (REAL-TIME STREAM)</div>
                 <div style={{ fontSize: '12px', marginBottom: '4px' }}>[INFO] {new Date().toISOString()} - Instância {selectedInstance?.instanceName} operacional.</div>
                 <div style={{ fontSize: '12px', marginBottom: '4px' }}>[INFO] {new Date().toISOString()} - Mensagem transacional despachada: 5511...</div>
                 <div style={{ fontSize: '12px', marginBottom: '4px' }}>[INFO] {new Date().toISOString()} - Heartbeat recebido do cluster Evolution.</div>
                 <div style={{ fontSize: '12px', color: '#64748B' }}>[DEBUG] Webhook response: 200 OK via HubCore Proxy.</div>
              </div>
           </div>

           <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ padding: '20px', borderRadius: '20px', border: '1px solid var(--border)', backgroundColor: '#F8FAFC' }}>
                 <h5 style={{ margin: '0 0 12px 0', fontSize: '11px', fontWeight: '600', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>VINCULAÇÃO NO HUB</h5>
                 <div style={{ fontSize: '14px', fontWeight: '500', color: 'var(--text-main)' }}>Empresa: {selectedInstance?.name}</div>
                 <div style={{ fontSize: '11px', color: '#94A3B8', marginTop: '4px' }}>UUID: {selectedInstance?.company_id}</div>
                 <button style={{...styles.actionBtn, width: '100%', marginTop: '16px', justifyContent: 'center', padding: '12px', fontWeight: '600'}}>
                    <ExternalLink size={16} /> VER GESTÃO DO CLIENTE
                 </button>
              </div>

              <div style={{ padding: '20px', borderRadius: '20px', border: '1px solid #fee2e2', backgroundColor: '#fff' }}>
                 <h5 style={{ margin: '0 0 12px 0', fontSize: '11px', fontWeight: '600', color: '#EF4444', textTransform: 'uppercase', letterSpacing: '0.8px' }}>ZONA DE SEGURANÇA</h5>
                 ...
                 <button style={{...styles.actionBtn, width: '100%', color: '#EF4444', borderColor: '#FEE2E2', justifyContent: 'center', padding: '12px', fontWeight: '600'}} onClick={() => toastError('O reset de webhooks é uma ação master. Confirme com o suporte técnico.')}>
                    <RefreshCw size={16} /> RESETAR TODOS OS WEBHOOKS
                 </button>
              </div>
           </div>
        </div>
