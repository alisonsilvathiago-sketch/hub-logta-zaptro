...
serve(async (req) => {
  ...
  try {
    ...
    if (path === 'create-payment') {
      const { client, email, phone, cpfCnpj, amount, due_date, method, description, asaas_customer_id } = await req.json()
      
      if (!asaasKey) throw new Error('ASAAS_API_KEY not configured')

      // A. Cliente Asaas: ID explícito (checkout assinatura) ou busca/criação por e-mail
      let asaasCustomerId = ''
      const existingId =
        typeof asaas_customer_id === 'string' && asaas_customer_id.trim() && !asaas_customer_id.includes('placeholder')
          ? asaas_customer_id.trim()
          : ''
