...
import { supabase, supabaseUrl } from '@core/lib/supabase';
import { toastSuccess, toastError } from '@core/lib/toast';
import type { AsaasPixQrCode } from '@core/services/asaasService';
...
const PaymentCheckout: React.FC = () => {
  ...
  const [pixData, setPixData] = useState<AsaasPixQrCode | null>(null);
  ...
  const handleGeneratePayment = async () => {
    ...
    try {
      ...
      const asaasCustomerId =
        typeof order.asaas_customer_id === 'string' && order.asaas_customer_id.trim()
          ? order.asaas_customer_id.trim()
          : '';

      const response = await fetch(`${supabaseUrl}/functions/v1/hub-core/create-payment`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          email: order.company?.email || 'financeiro@logta.com.br',
          phone: order.company?.phone || '',
          cpfCnpj: order.company?.cnpj || order.company?.cpf_cnpj || '',
          amount: order.amount,
          due_date: due,
          method,
          description: `Mensalidade Logta Hub - ${order.company?.name}`,
          ...(asaasCustomerId && !asaasCustomerId.includes('placeholder')
      ...
      if (!result.success) throw new Error(result.error || 'Falha ao criar cobrança');

      if (paymentMethod === 'PIX' && result.pix_image && result.pix_code) {
        setPixData({
          encodedImage: result.pix_image,
          payload: result.pix_code,
          expirationDate: '',
        });
      } else if (paymentMethod === 'BOLETO' && result.bank_slip_url) {
        window.open(result.bank_slip_url, '_blank');
      } else if (result.link) {
        window.open(result.link, '_blank');
      }

      toastSuccess('Pagamento gerado com sucesso via Asaas (servidor seguro).');
    } catch (err: any) {
        ...
        <div style={styles.left}>
          <div style={styles.card}>
            ...
            <div style={styles.security}>
               <Lock size={14} /> Cobrança criada no servidor (sem chave Asaas no navegador)
            </div>
          </div>
        </div>
