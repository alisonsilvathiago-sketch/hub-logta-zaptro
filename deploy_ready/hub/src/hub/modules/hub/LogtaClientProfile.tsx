        ...
        {activeTab === 'operacoes' && (
          <div style={s.tabContent}>
            <div style={{ ...s.card, marginBottom: 20 }}>
              <h3 style={s.cardTitle}>Controle Hub → LOGTA (plano, trial, IA, LogDrive)</h3>
              <p style={s.snapSub}>
                Publica políticas no app LOGTA via popup seguro. Ajuste o e-mail do cliente acima para usar como{' '}
                <strong>tenantKey</strong>.
              </p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 14 }}>
                <button
                  type="button"
                  style={{ ...s.editBtn, backgroundColor: '#2563EB' }}
                  onClick={() => pushEntitlements(defaultTrialEntitlements(tenantKey, trialEnds()))}
                >
                  Publicar trial (5 dias)
                </button>
                <button
                  type="button"
                  style={{ ...s.deleteBtn, border: '1px solid #FECACA', color: '#B91C1C' }}
                  onClick={() =>
                    pushEntitlements(expiredEntitlements(defaultTrialEntitlements(tenantKey, new Date(0).toISOString())))
                  }
                >
                  Simular expiração / bloqueio
                </button>
                <button
                  type="button"
                  style={{ ...s.chatBtn, backgroundColor: '#10B981', color: '#fff' }}
                  onClick={() =>
                    pushEntitlements(
                      activePlanEntitlements(defaultTrialEntitlements(tenantKey, trialEnds()), 'growth'),
                    )
                  }
                >
                  Ativar plano (liberar tudo)
                </button>
              </div>
            </div>
